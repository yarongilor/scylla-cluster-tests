...
from sdcm.db_stats import generate_node_capacity_query_postfix, get_used_capacity_gb, get_filesystem_total_size_gb, \
    AVAIL_SIZE_METRIC, AVAIL_SIZE_METRIC_OLD, GB_SIZE
...
class IcsSpaceAmplificationTest(LongevityTest):
    ...
    def _get_max_used_capacity_over_time_gb(self, node, start_time) -> float:
        ...
        :return:
        """
        fs_size_gb = get_filesystem_total_size_gb(self.prometheus_db, node=node)
        end_time = time.time()
    ...
    def _get_nodes_used_capacity(self):
        ...
        for node in self.db_cluster.nodes:
            dict_nodes_used_capacity[node.private_ip_address] = get_used_capacity_gb(self.prometheus_db, node=node)
