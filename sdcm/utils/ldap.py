...
class LdapContainerMixin:  # pylint: disable=too-few-public-methods
    ...
    def is_ldap_connection_bound():
        res = LdapContainerMixin.ldap_conn.bound if LdapContainerMixin.ldap_conn else False
        LOGGER.debug(f"is_ldap_connection_bound res: {res}")
        return res

    @staticmethod
    @retrying(n=10, sleep_time=6, message="Could not add Ldap entry, retrying..", allowed_exceptions=(LdapServerNotReady,))
    def _retry_add_ldap_entry(ldap_entry):
        try:
            LdapContainerMixin.ldap_conn.add(*ldap_entry)
        except Exception as error:  # pylint: disable=broad-except
            LOGGER.error("[_retry_add_ldap_entry] Could not add Ldap entry: %s", error)
    ...
    def add_ldap_entry(ip, ldap_port, user, password, ldap_entry):
        ...
        if not LdapContainerMixin.is_ldap_connection_bound():
            LOGGER.debug("add_ldap_entry not is_ldap_connection_bound!")
