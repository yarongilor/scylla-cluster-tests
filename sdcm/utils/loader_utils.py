...
class LoaderUtilsMixin:
    ...
    def assemble_and_run_all_stress_cmd_by_ks_names(self, stress_queue, stress_cmd, ks_names=None):
        for ks_name in ks_names:
            params = {"keyspace_name": ks_name, "stress_cmd": stress_cmd, "round_robin": self.params.get("round_robin")}
            self._run_all_stress_cmds(stress_queue, params)
    ...
    def run_cs_user_profiles(
        self, cs_profiles: str | int | list, duration_per_cs_profile: str | list = None, stress_queue: list = None
    ):
        """
        ...
        for i, cs_profile in enumerate(cs_profiles):
            ...
            with open(cs_profile, encoding="utf-8") as file:
                ...
                for cmd in [line.lstrip("#").strip() for line in content if line.find("cassandra-stress") > 0]:
                    stress_cmd = (
                        (cmd.format(profile_dst))
                        if duration_per_cs_profile is None
                        else (cmd.format(profile_dst, duration_per_cs_profile[i].strip()))
                    )
            ...
            if roles and "<sla credentials " in cmd:
                if "user=" in cmd:
                    ...
                    raise EnvironmentError(
                        "Stress command is defined wrong. Credentials already applied. Remove "
                ...
                if role_index is None:
                    # if stress command is not defined as expected, stop the tests and fix it. Then re-run
                    raise EnvironmentError(
                        "Stress command is defined wrong. Expected pattern '<credentials \\d>' was "
