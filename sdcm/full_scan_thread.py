...
class FullScanThread:
    ...
    def randomly_form_cql_statement(self, ks_cf: str, db_node: BaseNode) -> Optional[str]:
        ...
        if query_mode == 'reversed_query_options':
            if pks := get_partition_keys(ks_cf=ks_cf, db_node=db_node):
                partition_key = random.choice(pks)
                cmd = random.choice(self.query_options['reversed_query_options']).format(ks_cf, partition_key)
            else:
                self.log.info('No partition keys found for table: %s! A reversed query cannot be executed!', ks_cf)
                return None
        else:
